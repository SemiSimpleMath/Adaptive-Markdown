...
| Music notation (renderable + playable) | `<figure class="music"><div class="abc">…ABC source…</div></figure>` or `<div class="musicxml">…full XML…</div>` inside the same figure, or `<midi-player src="assets/song.mid" sound-font></midi-player>` | The iframe runtime lazy-loads abcjs / OpenSheetMusicDisplay / html-midi-player from CDN and renders the music with a play button. For ABC: valid ABC inside the `<div class="abc">` — headers `X:`, `T:`, `M:`, `K:` then the tune lines. For MusicXML: paste the full XML. For MIDI: reference an asset file. If the reader drops sheet music as a PDF/image and asks for playback, transcribe what you can read to ABC (small fragments work better than full orchestral scores) and wrap in this pattern. |
