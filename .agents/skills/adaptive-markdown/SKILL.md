...
- **The writable paths are `docs/<slug>/current.md` and `docs/<slug>/skills/<skill-slug>.md`.** Every doc lives in its own folder under `docs/`: `baseline.md` is the immutable history-0 (do not Edit it), `snaps/` is backend-managed snapshot state (do not Edit it), `assets/` is for materials the reader provides, and `skills/` holds the doc's sidecar skill files (next bullet). The pre-edit hook rejects any other write with a clear reason — refusing earlier in chat is cleaner than getting a hook error.
- **Per-doc skills live as sidecar files: `docs/<slug>/skills/<skill-slug>.md`.** One file per skill — frontmatter with `name:` (and optionally `description:`), then the instructions as markdown. They are the doc's working contract — voice, formatting rules, structural conventions, domain vocabulary specific to *that* doc — and they override generic guidance here when they conflict. They are agent configuration, NOT document content: never copy them into `current.md`. When the reader asks you to remember a doc-specific convention, write it to a skill file (create `skills/` if needed; slug = kebab-case of the name). When working on a doc folder, read `skills/*.md` first if your runtime hasn't already inlined them.
...
| Per-doc agent skill — LEGACY in-body form | `<section class="agent-skill">…</section>` | Older docs carry their working contract inline; the Doc view hides these sections via CSS, and exports strip them. Still authoritative when present — but do NOT create new ones: new per-doc skills are sidecar files at `docs/<slug>/skills/<skill-slug>.md` (see the writable-paths note above). Offer to migrate an in-body section to a file when you touch one (the viewer's Skills tab has Move to file). |
...
| Chart (Vega / Vega-Lite) | `<figure class="vega"><script type="application/json">{"$schema": "https://vega.github.io/schema/vega-lite/v5.json", "mark": "bar", "encoding": {"x": {"field": "a"}, "y": {"field": "b"}}, "data": {"values": [...]}}</script></figure>` | The iframe runtime lazy-loads vega + vega-lite + vega-embed (~800KB total) from CDN and renders into a sibling `.vega-render` div via `vegaEmbed`. Body is a JSON spec — auto-detected as Vega-Lite (concise, declarative) or Vega (low-level, more control) by the `$schema` field. Vega-Lite is usually the right choice: shorter specs, sensible defaults, grammar-of-graphics conventions. Mark types: `bar`, `line`, `point`, `area`, `rect`, `circle`, `tick`, `arc`, `boxplot`, `errorbar`. Inline data goes in `data.values`; for larger sets, `data.url` works if you've dropped a CSV/JSON into the doc's assets dir. Mutation: edit the script's `textContent` then `await __doc.rerender(figureEl)`. For incremental updates without rewriting source, `__doc.getRenderer(fig).instance` is the Vega `View` — `.signal(name, value).run()`, `.data(name, newRows).run()`, etc. Vega-Lite vs Plotly: prefer Vega-Lite for statistical charts, faceted small-multiples, layered encodings; prefer Plotly for interactive 3D, dashboards with sliders, scientific/financial domains where Plotly has specialized chart types. |
