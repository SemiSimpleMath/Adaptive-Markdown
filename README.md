...
- **The agent has shell access — sandboxed by default.** Reaching for `Bash` unlocks structured-data work that pure text editing can't do ergonomically: transposing MusicXML, pivoting CSV, resizing images, converting audio. Enforcement is platform-split. **macOS / Linux / WSL2:** the Claude CLI's built-in sandbox closes network by default and syscall-filters file access. **Windows:** the CLI no-ops, so we wrap each `Bash` call in a subprocess with curated env (no `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / proxy vars), scoped cwd (`docs/<slug>/`), and timeout — best-effort, not airtight. Treat the Windows fallback as a personal-machine tool. The load-bearing protection there is the env curation (nothing for a prompt-injected agent to exfiltrate), not the binary allowlist. `WebFetch` is denied on both platforms; if a task needs the open internet, drop the file in instead. Power-user opt-out: `python start.py --unsafe-bash` (Windows only) disables the wrapper — out-of-band only, no in-chat path can enable it.
...
python start.py --unsafe-bash  # (Windows only) disable the agent-Bash sandbox wrapper
```

Open <http://127.0.0.1:8090> in your browser. The tutorial doc loads by default.

The agent's `Bash` tool is **sandboxed automatically** — no flag needed. On macOS / Linux / WSL2 the Claude CLI's built-in sandbox handles it; on Windows a subprocess wrapper (curated env, scoped cwd, timeout) kicks in via a PreToolUse hook. `--unsafe-bash` disables the wrapper on Windows when you want the agent to have unfettered shell access in a workspace you fully trust — see [Security & responsible use](#security--responsible-use).
...
| `AM_UNSAFE_BASH` | Claude | `0` | (Windows only) When `1`, disables the subprocess sandbox wrapper around the agent's `Bash` tool — commands run with the backend's full env (incl. API keys), unrestricted filesystem access, and open internet. Set indirectly via `python start.py --unsafe-bash`. No effect on macOS / Linux / WSL2, where the CLI's built-in sandbox always enforces. |
