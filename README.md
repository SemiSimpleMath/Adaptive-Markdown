...
- **Inline editing** — hover any block (paragraph, heading, list, table, fenced code, or a whole figure) and a small toolbar appears: **✎** edits its **markdown source** in place, **＋** inserts a new block below, **🗑** deletes it. Because you edit the source (not the rendered view), styled spans, math, and inline formatting round-trip exactly — nothing is lost in a DOM→markdown reconstruction.
