...
**Minting is eager and system-maintained.** Whenever the runtime writes `current.md` (import, agent turn, inline edit, restore, startup backfill) it stamps a tracking ID before every top-level block that lacks one — paragraphs, headings, lists, tables, blockquotes, fenced code, and whole structural `<figure>` / `<section>` units alike — so any block can be located by a stable identity instead of by fuzzy text matching. Block boundaries come from the same markdown grammar the viewer renders with, so the ID always lands on a block's outer *start* and never inside a fenced code block, a `<script>` / `<style>` / `<pre>` body, or a figure's interior (where a comment could break a fence or fail JS validation). The agent never hand-authors these IDs; the system does, and the agent only preserves them. Because they're HTML comments they stay invisible in every renderer and are dropped by every converter, so the source still degrades gracefully to clean markdown.
